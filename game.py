...
from settings import CARDS_ENABLED       # <<< NOUVEAU
from ai_llm import get_ai_move           # <<< NOUVEAU
...
class Game:
    def __init__(self, screen):
        ...
        self.player.name = self.playerChosenName if hasattr(self, "playerChosenName") else "Joueur"
        self.enemy.name = self.enemyChosenName if hasattr(self, "enemyChosenName") else "IA"
    ...
    def ai_play(self):
        ...
        # 1) on récupère un tir proposé par l’IA
        row, col = get_ai_move(self.player.grid)

        # 2) sécurité au cas où
        ...
        for row in range(GRID_SIZE):
            for col in range(GRID_SIZE):
                ...
                if val == 0:
                    ...
                    color = (0,255,0) if show_ships else (200,200,200)
                elif val == -1:
                    color = (255,0,0)
                elif val == -2:
                    color = (0,0,255)
    ...
    def draw_cards(self, player):
        if not CARDS_ENABLED:
            return  # pas d'affichage si désactivé

        x, y = 50, 520
        for card in player.cards:
            rect = pygame.Rect(x, y, 120, 40)
            color = (255,215,0) if card != self.selected_card else (255,100,0)
            pygame.draw.rect(self.screen, color, rect)
