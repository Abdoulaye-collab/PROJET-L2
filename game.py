...
class Game:
    def __init__(self, screen):
        ...
        self.extra_shot = 0  # pour Double Tir
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN and not self.winner:
            ...
            for card in self.player.cards:
                ...
                if rect.collidepoint(x, y) and CARDS_ENABLED:
    ...
    def draw_grid(self, player, offset_x, show_ships=True):
        for row in range(GRID_SIZE):
            for col in range(GRID_SIZE):
                ...
                # couleur de fond / bateaux / touchés
                if val == 0:
                    color = (0,105,148)  # eau
                elif val == 1:
                    ...
                    color = (0,105,148)  # eau touchée = bleu
                else:
                    color = (0,105,148)
    ...
    def draw(self):
        ...
        # texte du tour centré
        status_text = "À ton tour !" if self.player_turn else "Tour de l'IA..."
        status_surf = self.font.render(status_text, True, (0,255,0) if self.player_turn else (255,0,0))
        screen_width = self.screen.get_width()
        self.screen.blit(status_surf, ((screen_width - status_surf.get_width()) // 2, 50))
